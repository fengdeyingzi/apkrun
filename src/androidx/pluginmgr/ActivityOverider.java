...
public class ActivityOverider {
	...
	public static ComponentName overrideStartService(Activity fromAct,String pluginId,Intent intent) {
		...
		PlugInfo plugin = con.getPluginByPackageName(fromAct.getPackageName());
		
		PluginContextWrapper pcw=new PluginContextWrapper(fromAct,plugin);
		
		//启动，Service,不支持，返回
		//Log.e("加载Service","加载Service:"+fromAct.getPackageName());
		

		AlertDialog.Builder ab=new AlertDialog.Builder(pcw.getApplicationContext());
	
		ab.setMessage("已拒绝启动启动Sevice:"+fromAct.getPackageName());
		...
		ab.show();
	//	FormatFaUtils.Fdialog(  plugin.getApplication() ,"提示","软件将启动一个Service,由于不支持,将会拒绝^_^","嗯",null,null,null);
		//Toast.makeText(plugin.getApplication(),"以停止Service:"+fromAct.getPackageName(),2000).show();
		//TODO 覆盖 StarSerTiastvice 方法
