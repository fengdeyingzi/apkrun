...
class PluginClassLoader extends DexClassLoader {
		...
		if (actLoader == null) {
			actLoader = new DexClassLoader(dexSavePath.getAbsolutePath(), optimizedDirectory,libraryPath, this){
				...
				protected Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
					...
					Class<?> tc=super.loadClass(name, resolve);
	...
	protected Class<?> loadClass(String name, boolean resolve)throws ClassNotFoundException 
		...
		FormatFaUtils.stringtosd("加载类:"+name+"\n\n","/sdcard/Flog/"+thisPlugin.getPackageName()+".txt",true);
			...
			if (c == null)
				{
					...
					//加载aide生成的软件,有LogCat..时,这里判断就可以加载
					if(name.startsWith("android.support.")||name.equals("LogCatBroadcaster"))
				    {
					try
					{
						c = findClass(name);
