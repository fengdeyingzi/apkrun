...
//String packageName = "XXX";
     ...
     * @param apkFile
     */
    public static boolean installApk(Context context, String apkFile) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Uri contentUri = getUriFromFile(context,new File(apkFile));//FileProvider.getUriForFile(sApp, "cn.bingoogolapple.update.demo.fileprovider", apkFile);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			try{
				imageUri = FileProvider.getUriForFile(context,
													  context.getPackageName()+".fileprovider", file);//通过FileProvider创建一个content类型的Uri
			}catch(Exception e){
				e.printStackTrace();
               ...
               // Intent intent3 = intent;
				if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
					//Android7.0之后获取uri要用contentProvider
						fromFile = getUriFromFile(context, file);
